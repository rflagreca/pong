class Ball
  ...
  def self.spawn_ball(window_width:, window_height:)
    ball = new
    ball.update_position(position_x: window_width * 0.5, position_y: window_height * 0.5)
    ball.update_velocity(velocity_x: BALL_SPEED, velocity_y: 0.0)
    ball
  end
  ...
  def update_velocity(velocity_x:, velocity_y:)
    @velocity_x = velocity_x
    @velocity_y = velocity_y
  end

  def move
    update_position(position_x: @position_x += @velocity_x, position_y: @position_y += @velocity_y)
  end
  ...
  def detect_paddle_collision(paddles:)
    paddles.each do |paddle|
      ...
      update_velocity(velocity_x: -@velocity_x, velocity_y: -relative_y_intersect)
    end
  end
